...
export function parseMarkdown(text: string): string {
  ...
  html = html.replace(
    /^(?!<div|<\/div|<table|<\/table|<thead|<\/thead|<tbody|<\/tbody|<tr|<\/tr|<th|<\/th|<td|<\/td)(.*)$/gm,
    (match) => {
      if (
...
function parseTable(rows: string[], escape: (str: string) => string): string | null {
  if (rows.length < 2) return null;

  // Check if second row is separator (|---|---|)
  const separatorRow = rows[1];
  if (!separatorRow || !/^\|[\s\-:]+(\|[\s\-:]+)+\|$/.test(separatorRow)) return null;
